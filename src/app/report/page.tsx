// src/app/report/page.tsx – version finale 100 % build-proof
"use client";

import { useState, useEffect } from "react";
import { useSearchParams, useRouter } from "next/navigation";
import { ArrowLeft, Upload, Shield, CheckCircle2 } from "lucide-react";
...
const MOTIFS = [
  ...
  "Conduite sous alcool", "Conduite sous stupéfiants", "Feu rouge grillé", "Refus de priorité", "Délit de fuite"
];
...
export default function ReportPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!file || !sworn || motifs.length === 0 || !selectedType || !selectedColor || !ville || !plaque) return;

    setLoading(true);

    // Upload simulé (on mettra R2 plus tard)
    ...
    if (!error) {
      setSuccess(true);
      setTimeout(() => router.push(plaque ? `/p/${plaque}` : "/"), 3000);
    } else {
      console.error(error);
  ...
  return (
    <main className="min-h-screen bg-white px-6 pt-24 pb-20">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          {/* === TOUS LES CHAMPS IDENTIQUES À AVANT === */}
          {/* Je te les remets tous pour être sûr */}
            ...
            >
              {preview ? (
                <div className="space-y-4">
                  ...
                  <button type="button" onClick={() => { setFile(null); setPreview(null); }} className="text-red-600">
                    Supprimer
                  </button>
                </div>
              ) : (
                <>
                  ...
                  <p className="text-xl text-gray-600">Glisse ton fichier ici ou clique</p>
                  <input id="file-input" type="file" accept="image/*,video/*" onChange={(e) => e.target.files?.[0] && setFile(e.target.files[0])} className="hidden" />
                </>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ex: Paris 8e, A6 km 120"
            ...
            <div className="grid grid-cols-3 gap-3">
              {VEHICLE_TYPES.map((type) => (
                <button key={type} type="button" onClick={() => setSelectedType(type)}
                  className={`py-4 rounded-xl border-2 font-medium transition ${selectedType === type ? "bg-black text-white border-black" : "border-gray-300 hover:border-gray-500"}`}>
                  {type}
                </button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-4 gap-3">
              {COLORS.map((color) => (
                <button key={color} type="button" onClick={() => setSelectedColor(color)}
                  className={`py-8 rounded-xl border-2 font-medium transition relative overflow-hidden ${selectedColor === color ? "ring-4 ring-black ring-offset-4" : ""}`}
            ...
            <div className="grid grid-cols-2 gap-3">
              {MOTIFS.map((motif) => (
                <label key={motif} className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" checked={motifs.includes(motif)} onChange={(e) => e.target.checked ? setMotifs([...motifs, motif]) : setMotifs(motifs.filter(m => m !== motif))} className="w-6 h-6 rounded border-gray-300" />
          ...
          <button
            type="submit"
            disabled={loading || !file || !sworn || motifs.length === 0 || !selectedType || !selectedColor || !ville || !plaque}
