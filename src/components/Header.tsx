...
import { createServerClient } from "@/lib/supabase-server"; // ← la bonne fonction
import Image from "next/image";
import { LogOut, User, Car, Siren, LogIn } from "lucide-react";

export default async function Header() {
  // Cette fonction est faite pour les server components
  const supabase = createServerClient();
  const { data: { user } } = await supabase.auth.getUser();

  return (
    <header className="sticky top-0 z-50 w-full border-b border-b bg-white shadow-sm">
